...
class RouteProcessor:
    ...
    async def _process_mp_unreach(
        ...
        for item in withdrawn:
            if safi == SAFI.EVPN and isinstance(item, dict):
                # EVPN withdrawal
                route = self._create_evpn_route(router_ip, peer_ip, peer_as, item, {}, timestamp)
                route["is_withdrawn"] = True
                route["withdrawal_time"] = timestamp
            else:
                # Regular prefix withdrawal
                route = self._create_base_route(router_ip, peer_ip, peer_as, str(item), timestamp)
                route["family"] = family
                route["is_withdrawn"] = True
                route["withdrawal_time"] = timestamp
                route["afi"] = afi
    ...
    def _create_base_route(
        self, router_ip: str, peer_ip: str, peer_as: int, prefix: str, timestamp: datetime
    ) -> Dict[str, Any]:
    ...
    def _create_route_from_nlri(
        ...
        route["as_path"] = (
            json.dumps(attributes.get("as_path")) if attributes.get("as_path") else None
        )
        route["communities"] = (
            json.dumps(attributes.get("communities")) if attributes.get("communities") else None
        )
        route["extended_communities"] = (
            json.dumps(attributes.get("extended_communities"))
            if attributes.get("extended_communities")
            else None
        )
        route["large_communities"] = (
            json.dumps(attributes.get("large_communities"))
            if attributes.get("large_communities")
            else None
        )
        ...
        route["cluster_list"] = (
            json.dumps(attributes.get("cluster_list")) if attributes.get("cluster_list") else None
        )
    ...
    def _create_evpn_route(
        ...
        # Route Type 4 specific fields
        route["originating_ip"] = evpn_data.get("originating_ip")
        route["ip_length"] = evpn_data.get("ip_length")
