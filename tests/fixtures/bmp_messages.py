...
class BMPMessageBuilder:
    ...
    def create_per_peer_header(
        ...
        bgp_id_value = sum(
            int(octet) << (8 * (3 - i)) for i, octet in enumerate(peer_bgp_id.split("."))
        )
    ...
    def create_bgp_update(
        withdrawn: List[str] = None, path_attrs: List[Dict] = None, nlri: List[str] = None
    ) -> bytes:
        ...
        for attr in path_attrs:
            ...
            if attr_type == 1:  # ORIGIN
                ...
                attr_data += struct.pack(">BBB", 0x40, 2, len(as_path_data)) + as_path_data
            elif attr_type == 3:  # NEXT_HOP
                next_hop_bytes = struct.pack(
                    ">I",
                    sum(
                        int(octet) << (8 * (3 - i)) for i, octet in enumerate(attr_value.split("."))
                    ),
                )
    ...
    def create_route_monitoring_message(
        peer_ip: str = "192.0.2.1", peer_as: int = 65001, nlri: List[str] = None
    ) -> bytes:
        """Create route monitoring BMP message."""
        nlri = nlri or ["10.0.1.0/24"]

        # Create per-peer header
        peer_header = BMPMessageBuilder.create_per_peer_header(peer_ip=peer_ip, peer_as=peer_as)
        ...
        bgp_update = BMPMessageBuilder.create_bgp_update(path_attrs=path_attrs, nlri=nlri)
    ...
    def create_peer_up_message(
        peer_ip: str = "192.0.2.1", peer_as: int = 65001, local_ip: str = "192.0.2.100"
    ) -> bytes:
    ...
    def create_peer_down_message(
        peer_ip: str = "192.0.2.1", peer_as: int = 65001, reason: int = 1
    ) -> bytes:
    ...
    def create_initiation_message(
        system_name: str = "test-router", system_descr: str = "Test BMP Implementation"
    ) -> bytes:
    ...
    def create_stats_report_message(
        peer_ip: str = "192.0.2.1", peer_as: int = 65001, stats: List[Dict] = None
    ) -> bytes:
